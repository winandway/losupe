...
```mermaid
flowchart LR
  A[Franja 7:00 / 12:00 / 17:00<br/>hora del Este] --> B{¿Robot encendido?<br/>¿Llaves? ¿Tope? ¿Cupo?}
...
| Correr                          | **Robot** (franjas 7:00 / 12:00 / 17:00 hora del Este) o Richard con **Ejecutar ahora (1 nota)** | Alterna encargo/universal; anota la corrida.                                                                                                                   | Inicio → «Última corrida» muestra la nota, el costo o el error.                       |
...
| ----------------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Entrada           | `worker.ts` → `src/lib/robot/scheduled.ts`  | `/__scheduled` (cabecera `x-yad-cron` o `?key=CRON_SECRET`) y el cron (`0 11,12,16,17,21,22 * * *`, las dos horas UTC posibles de cada franja por el horario de verano). El turno se apunta por franja y día del Este, así que el segundo disparo no repite nota. |
